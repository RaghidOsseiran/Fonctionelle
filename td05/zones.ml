...
let make_rectangle width height = fun p ->   
  let width_p = realpart p and height_p = imagpart p in 
  if (width_p < 0. || width_p > width || height_p < 0. || height_p > height) then false else true


(* Given two zones, create a zone that behaves as the union of the two. *) (* let zone_union zone1 zone2 = *)
let zone_union zone1 zone2 = fun p -> 
...
let zone_difference zone1 zone2 = fun p -> 
  (point_in_zone_p p zone1 && not (point_in_zone_p zone2 p))

(* let make_disk radius center = *)

let make_disk radius center = fun p ->
  let translated_p = (c_dif p center) in 
  c_abs translated_p <= radius

let test_make_disk () = fun _ -> 
  begin 
    let c = make_disk 2. (make_point 3. 3.) in
    let c1 = translate_zone c (make_point (-1.) (-1.)) in
    assert (not (point_in_zone_p (make_point 1. 1.) c));
    assert (not (point_in_zone_p (make_point 1. 1.) (zone_intersection c c1)));
  end
...
(*=> On doit bien se rapelle que zone: point -> bool, et que la fonction point_in_zone_p prend un point, une zone, et apelle
 ...
 fait elle meme un apelle a point_in_zone_p mais avec la zone qui lui a ete passer en parametre au debut.*)
